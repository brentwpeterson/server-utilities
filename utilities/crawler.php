...
/**
 * use Curl to make sure we can download from all sites
 * patched by atheotsky
 */
function getWebPage($url, $timeout = 120) {
    $options = array(
...
function file_get_html($url, $use_include_path = false, $context=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
{
    ...
    $contents = getWebPage($url); // use curl to make it work with most web pages
...
/*=====================================CUSTOM CRAWLER START HERE=====================================*/

/**
 * @package     php cli crawler
 * @author      atheotsky
 * @copyright   Copyright (c) 2014 Thanh, Nguyen Duy http://invisiblezero.net
 */

abstract class Crawler_Shell_Abstract
{
    protected $_rootPath;
     ...
     */
    protected function _applySitemapxml()
    {
        $sitemap = $this->_getRootPath() . 'sitemap.xml';
        if (file_exists($sitemap)) {
            $xml = new SimpleXMLElement(file_get_contents($sitemap));
            foreach ($xml->url as $url_list)
            {
                $this->_sitemap[] = $url_list->loc->__toString();
            }
        }
    }
 ...
 * Crawler Shell Class
 *
 * @category    php cli crawler
 * @author      atheotsky
 * @copyright   Copyright (c) 2014 Thanh, Nguyen Duy http://invisiblezero.net
 */
class Crawler_Shell_Run extends Crawler_Shell_Abstract {
    /**
     * default interval is 0 second
     */
    protected $interval = 0;
    /**
     * default depth level is 0
     */
    protected $depth = 0;
    /**
     * crawler output doesn't show by default
     */
    protected $verbose = false;
    /**
     * exclude
     */
    protected $exclude = array('jpg', 'png', 'jpeg', 'pdf', '7z', 'zip', 'rar', 'mp3', 'aac', 'mp4', 'apk', 'bat', 'tar', 'swf', 'iso');

    /**
     * storage for urls
     */
    protected $crawled_urls = array();
    ...
    public function init()
    {
        ...
        /*initialize options*/
        if ($this->getArg('depth')) {
            $this->depth = intval($this->getArg('depth'));
        }
        if ($this->getArg('interval')) {
            $this->interval = intval($this->getArg('interval'));
        }
        if ($this->getArg('verbose')) {
            $this->verbose = (boolean) $this->getArg('verbose');
        }
        if ($this->getArg('exclude')) {
            $this->exclude = explode(";", $this->getArg('exclude'));
        }
    ...
    protected function rel2abs($rel, $base)
    {
        ...
        if (empty($scheme) && $empty($host)) extract(parse_url($this->fallback_base));
     ...
     * rebuild url if 
     */
    protected function perfect_url($u, $base){
        $bp = parse_url($base);
        $bp['scheme'] = empty($bp['scheme']) ? null : $bp['scheme'];
        $bp['host'] = empty($bp['host']) ? null : $bp['host'];
        $bp['path'] = empty($bp['path']) ? null : $bp['path'];
    ...
    public function run()
    {
        if (!empty($this->_sitemap) || $this->getArg('website')) {
            $urls = empty($this->_sitemap) ? array($this->getArg('website')) : $this->_sitemap;
            foreach ($urls as $url) {
                $this->crawl($url);
            }
        }
     ...
     */
    protected function crawl($url, $depth = 0)
    {
        /*validate before digging into url*/
        if ($depth > $this->depth) return;
        $ext = strtolower(pathinfo($url, PATHINFO_EXTENSION));
        if (in_array($ext, $this->exclude)) return;

        $uen = urlencode($url);
        if (empty($this->crawled_urls[$uen])) {
            /*update fallback_base*/
            if (filter_var($url, FILTER_VALIDATE_URL)) $this->fallback_base = $url;

            /*download html content, parse then save it to $html object*/
            $html = file_get_html($url);
            if ($html) {
                if ($this->verbose) echo "{$url}\n";
                if ($this->interval) usleep($this->interval * 1000000);

                $this->crawled_urls[$uen] = date("YmdHis");

                foreach($html->find('a') as $a_tag){
                    $_url = $this->perfect_url($a_tag->href, $url);
                    if (empty($_url)) continue;
                    $enurl = urlencode($_url);
                    if($_url != '' && substr($_url,0,4) != 'mail' && substr($_url,0,4) != 'java'){
                        $this->crawl($_url, $depth + 1);
                    }
                }
            }
            else {
                if ($this->verbose) echo "{$url} IS NOT AVAILABLE!!!\n";
            }
        }
    }
...
Usage:  php -f crawler.php -- [options]
  ...
  -website <website>           Website url for input. Will be ignored if there is ./sitemap.xml
